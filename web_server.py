...
def interpolate_free_draw_data(poly_order=1):
    ...
    for store in [chrg_dat, eps_dat]:
        ...
        spl = ip.UnivariateSpline(xx_unq[xx_unq.argsort()],
                                  ...
                                  s=0, k=poly_order, ext='const')
        store.data = dict(xs=[zz], ys=[spl(zz)])

    pmf_spls = []
    for i in range(2):
        xx, yy = pmf_dat.data['xs'][i], pmf_dat.data['ys'][i]  # interpolate pmf data
        xx_unq, idx_unq = np.unique(xx, return_index=True)
...
charge = figure(plot_width=300, plot_height=150, y_axis_label='\u03C1(z) [e/m\u00B3]',
                tools='reset,save', y_range=(-2, 2))
epsilon = figure(plot_width=300, plot_height=150, x_range=charge.x_range, y_axis_label='\u03B5(z)',
                 tools='reset,save', y_range=(0, 100))
pmfs = figure(plot_width=300, plot_height=150, x_range=charge.x_range, x_axis_label='z-distance [nm]',
